...
def clear_frameOutput():
    for file_name in os.listdir('frameOutput/'):
        file_path = os.path.join('frameOutput/', file_name)
        if os.path.isfile(file_path) and file_name.endswith(".png"):
            os.remove(file_path)
    return


def array_to_png(array, filename="test", folder="frameOutput", mode="RGB"):
    """
    Modes:
        - RGB
        - RGBA
...
def make_mp4(name=None, folder="frameOutput", vid_folder=".", fps=30):
    if name is None:
...
def render_video(step, frames=90, fps=30, mode="RGB", video_folder=".", video_name=None):
    print()
    ...
    print('|' + ' '*78 + '|')
    make_mp4(name=video_name, fps=fps, vid_folder=video_folder)
    print('|' + ' '*78 + '|')
    print(f'[-{datetime.now().strftime("%y/%m/%d-%H:%M:%S"):->76}-]')
    print()
    print()
...
if __name__ == "__main__":
    print('hi :3')
    clear_frameOutput()
    print('cleared frameOutput folder')
